    ...
    try {
      ...
      if (page && limit && (status)) {
        ...
        const data = await InspeksiOutsourcing.findAll({
          ...
          where: obj,
          include: [
            { model: InspeksiOutsourcingPoint, as: "inspeksi_outsourcing_point" },
        ...
        const data = await InspeksiOutsourcing.findAll({
          order: [["createdAt", "DESC"]],
          include: [{ model: InspeksiOutsourcingPoint, as: "inspeksi_outsourcing_point" },
          { model: Users, as: "inspector" }],
          offset,
          limit: parseInt(limit),
        });
  ...
  submitInspeksiOutsourcing: async (req, res) => {
    try {
      ...
      const {
        ...
        status_outsourcing,
        lama_pengerjaan,
        inspeksi_outsourcing_point
      } = req.body


      await InspeksiOutsourcing.update({ outsourcing, kesesuaian, status_outsourcing, lama_pengerjaan,status:"history" }, {
        where: {
          id
        }
      })
      for (let i = 0; i < inspeksi_outsourcing_point?.length; i++) {
        await InspeksiMasterPointOutsourcing.update({
          hasil: inspeksi_outsourcing_point[i].hasil,
          foto: inspeksi_outsourcing_point[i].foto
        }, {
          where: { id: inspeksi_outsourcing_point[i].id }
        })
      }

      res.status(200).json({ msg: "submit successful" })
    } catch (err) {
      res.status(500).json({ msg: err.message });
    }
  }
