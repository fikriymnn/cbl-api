...
const inspeksiBahanController = {
    getInspeksiBahan: async (req, res) => {
        try {
            ...
            if (page && limit && status) {
                ...
                const data = await InspeksiBahan.findByPk(id, { include: { model: InspeksiBahanResult, as: "inspeksi_bahan_result" } })
                let array = []
    ...
    createInspeksiBahan: async (req, res) => {
        try {
            ...
            const { tanggal, no_surat_jalan, supplier, jenis_kertas,
                ukuran, jam, inspector, jumlah } = req.body
            ...
            const data = await InspeksiBahan.create({
                tanggal, no_surat_jalan, supplier, jenis_kertas,
                ukuran, jam, inspector, jumlah
            })
            if (data) {
                const array = []
...
const master_result_fix = [
    ...
    },
    {
        ...
        metode: "Potongan kertas ukuran 10x10cm area KIRI,TENGAH, KANAN|Timbang masing-masing beratnya|Jumlahkan dan hitung nilai rata ratanya (KIRI<TENGAH<KANAN:3)",
