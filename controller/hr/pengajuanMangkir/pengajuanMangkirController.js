...
const PengajuanMangkirController = {
  getPengajuanMangkir: async (req, res) => {
    ...
    if (start_date && end_date) {
      ...
      obj.tanggal = { [Op.between]: [startDate, endDate] };
    }
    try {
      if (page && limit) {
        const length = await PengajuanMangkir.count({ where: obj });
        const data = await PengajuanMangkir.findAll({
        ...
        const data = await PengajuanMangkir.findByPk(_id, {
          include: [
            {
              model: Karyawan,
              as: "karyawan",
  ...
  createPengajuanMangkir: async (req, res) => {
    const { id_karyawan, id_pengaju, tanggal } = req.body;
    ...
    try {
      ...
      const dataPengajuanMangkir = await PengajuanMangkir.create(
        {
          id_karyawan,
  ...
  approvePengajuanMangkir: async (req, res) => {
    const _id = req.params.id;
    const { catatan_hr } = req.body;
    const t = await db.transaction();

    try {
      const dataPengajuanMangkir = await PengajuanMangkir.findByPk(_id);
      if (!dataPengajuanMangkir)
        return res.status(404).json({ msg: "data tidak di temukan" });

      await PengajuanMangkir.update(
        {
          status: "approved",
  ...
  rejectPengajuanMangkir: async (req, res) => {
    const _id = req.params.id;
    const { catatan_hr } = req.body;
    const t = await db.transaction();
...
module.exports = PengajuanMangkirController;
